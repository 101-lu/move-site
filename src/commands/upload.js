...
import { getAdapter, detectCMS } from '../cms/index.js';
import { SSHTransfer } from '../transfer/ssh.js';

/**
 * Upload files to a remote environment
 */
export async function runUpload(environment, options, config) {
  ...
  if (options.dryRun) {
    ...
    for (const [category, categoryFiles] of Object.entries(categories)) {
      if (categoryFiles.length > 0) {
        ...
        categoryFiles.slice(0, 5).forEach(f => {
  ...
  try {
    ...
    const result = await transfer.uploadFiles(
      files,
      envConfig.remotePath,
      (progress) => {
        if (progress.type === 'start') {
          process.stdout.write(`\r⏳ Uploading (${progress.completed + 1}/${progress.total}): ${progress.file.slice(0, 50)}...`);
        }
      }
    );
...
function categorizeFiles(files) {
  ...
  for (const file of files) {
    ...
    } else if (file.relativePath.startsWith('wp-admin') || 
               file.relativePath.startsWith('wp-includes')) {
